...
# A股法定休市日（不含周末，周末由weekday判断；每年初更新）
# 2026：元旦1/1-1/2 春节2/16-2/20 清明4/6 劳动节5/1,5/4-5/5
#       端午6/19 中秋9/25 国庆10/1-10/2,10/5-10/7
HOLIDAYS_2026 = {
...
def is_trading_day():
    """周一~周五且非法定休市日即视为交易日。
    旧逻辑用行情date==今天判断，盘前08:30接口date恒为上一交易日→晨报永远跳过。
    晨报/复盘用的本就是已收盘数据，无需等当日行情落地。"""
    now = datetime.now()
    if now.weekday() >= 5:
        return False
