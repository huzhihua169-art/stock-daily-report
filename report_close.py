...
3.5 **持仓专项监控（中天600522 + 光迅002281）**：对照持仓状态灯触发位距离。中天：减仓区36/铁底27.02，触及/接近（<5%）必须醒目置顶，写动作含义（减仓区=减100股首选路径；放量破27.02=处置）；光迅：成本232.64浮亏~-18%，触发位待研究卡片定稿，8/20中报<5.586亿否决持有。今日有持仓重大信息（公告、中报、龙虎榜、融资余额、海外政策、大额资金异动）单独列出给一句关键判断。提示距8/20光迅中报、8/28中天中报各剩几天。
