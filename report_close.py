...
6. **明日验证清单**：不超过5条具体可核验的事项（每一条都必须写成"可对错判断"的假设，如"XX板块明日上涨/下跌"，不要写模糊描述）。⚠️ 持仓标的（中天科技600522）**禁止**写成"明日收涨/收跌"式无后果假设，只写触发位判断：若现价在27-36持有区内，写"无动作——仍在27-36持有区，距减仓区还差X%、距铁底还有Y%缓冲"；仅当触及36或27.02时才写可验证动作假设。
"""
