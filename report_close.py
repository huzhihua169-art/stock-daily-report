...
3.5 **持仓专项监控（中天600522）**：对照持仓状态灯触发位距离（减仓区36/铁底27.02）。触及/接近任一触发位（<5%）必须醒目置顶，写动作含义（减仓区=减100股首选路径；放量破27.02=处置）。今日有中天重大信息（公告、中报、龙虎榜、融资余额、海外政策、大额资金异动）单独列出给一句关键判断。提示距8/28中报核验还剩几天。
